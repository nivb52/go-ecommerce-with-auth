...
func (c *Card) Charge(currency string, amount int) (*stripe.PaymentIntent, string, error) {
	return c.createPaymnetIntent(currency, amount)
}
func (c *Card) createPaymnetIntent(currency string, amount int) (*stripe.PaymentIntent, string, error) {
	stripe.Key = c.Secret
	...
	if err != nil {
		...
		if stripErr, ok := err.(*stripe.Error); ok {
			errorMsg = cardErrorMessage(stripErr.Code)
		}
